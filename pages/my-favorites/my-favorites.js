...
Page({
  ...
  onReachBottom() {
    ...
    callCommunity('getMyFavorites', { limit: PAGE_SIZE, skip })
      .then((data) => {
        const newPosts = normalizePosts(data.favorites || [])
        this.setData({
  ...
  async loadFirst() {
    ...
    try {
      const data = await callCommunity('getMyFavorites', { limit: PAGE_SIZE, skip: 0 })
      const posts = normalizePosts(data.favorites || [])
  ...
  async onUnfavorite(e) {
    ...
    if (!post) return
    try {
      await callCommunity('toggleFavorite', { postId: post._id })
