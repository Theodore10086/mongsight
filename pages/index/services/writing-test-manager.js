class WritingTestManager {
  ...
  updateContext(ctx, canvas) {
    this.ctx = ctx
    this.canvas = canvas
    if (this.inkCuring) this.inkCuring.setContext(ctx, canvas)
    if (this.particleSystem) this.particleSystem.setContext(ctx, canvas)
  }

  async handleStrokeComplete(strokes) {
    if (this.isProcessing || !strokes || strokes.length === 0) return
    this.isProcessing = true
    const lastStroke = strokes[strokes.length - 1]
    this.currentStrokeData = { strokes, lastStroke }
  ...
  _triggerTransition() {
    ...
    wx.vibrateShort({ type: 'heavy', success: () => { console.log('[WritingTestManager] Vibration triggered') } })
    this.onVibrate()
    setTimeout(() => {
      this.onShowBubble({ name: '蒙宝', message: this.welcomeMessage, avatar: '/assets/images/mengbao.jpg' })
      this.onTransitionComplete({ success: true, message: this.welcomeMessage })
      this.isProcessing = false
    }, 300)
  }

  getWelcomeMessage() { return this.welcomeMessage }
  setWelcomeMessage(message) { this.welcomeMessage = message }
  isAnimating() { return this.isProcessing || (this.inkCuring && this.inkCuring.isCuring) || (this.particleSystem && this.particleSystem.isEmitting()) }
  reset() {
    if (this.inkCuring) this.inkCuring.reset()
