class InkCuringManager {
  ...
  getStrokeCenter(stroke) {
    ...
    let sumX = 0, sumY = 0
    stroke.forEach(point => { sumX += point.x; sumY += point.y })
    return { x: sumX / stroke.length, y: sumY / stroke.length }
  }

  getStrokeFeature(stroke) {
    if (!stroke || stroke.length < 2) return { velocity: 0.5, pressure: 0.5, intensity: 0.5 }
    let totalDistance = 0, totalTime = 0, pressures = []
    ...
    return { velocity: normalizedVelocity, pressure: normalizedPressure, intensity: (normalizedVelocity + normalizedPressure) / 2 }
  }

  async cure() {
    if (this.isCuring || this.originalStrokes.length === 0) return null
    this.isCuring = true
    ...
    return new Promise((resolve) => {
      const animate = () => {
        ...
        const currentOpacity = this.config.opacityStart + (this.config.opacityEnd - this.config.opacityStart) * easedProgress
        const currentGrayscale = this.config.grayscaleStart + (this.config.grayscaleEnd - this.config.grayscaleStart) * easedProgress
        ...
        if (progress < 1) {
          ...
          resolve({ center, feature, strokes: this.curedStrokes })
        }
      }
      animate()
    })
