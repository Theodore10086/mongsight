...
class ParticleSystem {
  ...
  _randomColor(feature) {
    ...
    if (!feature) return this.config.colors.gold
    const { velocity, pressure, intensity } = feature
    if (intensity > 0.7) return this.config.colors.gold
    else if (intensity > 0.4) return velocity > 0.5 ? this.config.colors.amber : this.config.colors.jade
    else return pressure > 0.5 ? this.config.colors.cyan : this.config.colors.mist
  }
  ...
  _createBurstParticles(centerX, centerY, feature) {
    ...
    for (let r = 0; r < ringCount; r++) {
      ...
      const ringParticle = new Particle(centerX, centerY, {
  ...
  emit(centerX, centerY, feature) {
    ...
    const animate = () => {
      ...
      this.particles.forEach(particle => { particle.update(); particle.draw(this.ctx) })
