...
class LevelSystem {
  ...
  _getNextLevel() {
    ...
    if (currentIndex < LevelConfig.LEVELS.length - 1) {
      ...
      return {
        name: next.name,
        title: next.title,
        requiredScore: next.minScore,
        remaining: next.minScore - this.totalScore
      }
    }
    return null
  }
  ...
  async addScore(score) {
    ...
    const oldMaterialLevel = this.materialLevel
    ...
    if (newMaterial && newMaterial !== oldMaterial) {
      ...
      this.onMaterialUpgrade({
        from: oldMaterial,
        to: newMaterial,
        level: this.materialLevel,
        info: materialInfo
      })
    }
  ...
  _updateStreak() {
    ...
    try {
      wx.setStorageSync(LevelConfig.STREAK_KEY, {
        lastDate: today,
        streak: this.streak
      })
    } catch (e) {
